...
async def test_insert_and_get_history_filters_deleted() -> None:
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=10, thread_id=2001,
            from_user_id=1, text="hi", date_unix=100,
        ),
        [
            InsertMessageBoxRow(
                user_id=1, user_message_box_id=1, dialog_message_id=10,
                out=True, random_id=None, pts=1,
            ),
        ],
    )
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=11, thread_id=2001,
            from_user_id=2, text="hi back", date_unix=101,
        ),
        [
            InsertMessageBoxRow(
                user_id=1, user_message_box_id=2, dialog_message_id=11,
                out=False, random_id=None, pts=2,
            ),
        ],
    )

    rows = await dao.get_history(user_id=1, thread_id=2001, limit=10)
...
async def test_find_by_random_id_idempotency() -> None:
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=20, thread_id=2001,
            from_user_id=1, text="hello", date_unix=200,
        ),
        [
            InsertMessageBoxRow(
                user_id=1, user_message_box_id=5, dialog_message_id=20,
                out=True, random_id=999, pts=3,
            ),
        ],
    )
...
async def test_mark_inbox_read_and_peer_outbox_for_inbox() -> None:
    ...
    # User 2 sends two messages to user 1; each message has two per-user copies.
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=30, thread_id=2001,
            from_user_id=2, text="m1", date_unix=300,
        ),
        [
            InsertMessageBoxRow(
            ...
            InsertMessageBoxRow(
                ...
                out=False, random_id=None, pts=1,
            ),
        ],
    )
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=31, thread_id=2001,
            from_user_id=2, text="m2", date_unix=301,
        ),
        [
            InsertMessageBoxRow(
                user_id=2, user_message_box_id=11, dialog_message_id=31,
                out=True, random_id=None, pts=2,
            ),
            InsertMessageBoxRow(
    ...
    receipts = await dao.peer_outbox_for_inbox(
        reader_user_id=1, thread_id=2001, max_inbox_ubid=21,
    )
    assert len(receipts) == 1
    rcp = receipts[0]
    assert rcp.sender_user_id == 2
    assert rcp.sender_thread_id == 2001
    assert rcp.max_outbox_id == 11

    n = await dao.mark_inbox_read(user_id=1, thread_id=2001, max_ubid=21)
    assert n == 2

    again = await dao.mark_inbox_read(user_id=1, thread_id=2001, max_ubid=21)
    assert again == 0


@pytest.mark.asyncio
async def test_get_box_returns_row() -> None:
    pool = FakeChatPool()
    pool.add_dialog(2001, owner_user_id=1, peer_id=2, is_group=False)
    dao = ChatDAO(pool)
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=99, thread_id=2001,
            from_user_id=1, text="hi", date_unix=10,
        ),
        [
            InsertMessageBoxRow(
...
@pytest.mark.asyncio
async def test_update_message_content_updates_single_message_row() -> None:
    pool = FakeChatPool()
    pool.add_dialog(2001, owner_user_id=1, peer_id=2, is_group=False)
    pool.add_dialog(2001, owner_user_id=2, peer_id=1, is_group=False)
    dao = ChatDAO(pool)
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=77, thread_id=2001,
            from_user_id=1, text="orig", date_unix=10,
        ),
        [
            InsertMessageBoxRow(
    ...
    n = await dao.update_message_content(
        77, "edited", None, edit_date=12345,
    )
    # One ``messages`` row updated — visible to every per-user copy via the JOIN.
    assert n == 1
...
async def test_mark_deleted_is_idempotent() -> None:
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=200, thread_id=2001,
            from_user_id=1, text="m", date_unix=1,
        ),
        [
            InsertMessageBoxRow(
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=201, thread_id=2001,
            from_user_id=1, text="n", date_unix=2,
        ),
        [
            InsertMessageBoxRow(
...
async def test_get_boxes_by_dialog_message_ids_skips_deleted() -> None:
    ...
    await dao.insert_message_with_boxes(
        None,
        InsertMessageRow(
            dialog_message_id=300, thread_id=2001,
            from_user_id=1, text="x", date_unix=1,
        ),
        [
            InsertMessageBoxRow(
