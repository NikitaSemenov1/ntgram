...
def test_bad_server_salt_includes_plaintext_session_id() -> None:
    ...
    inbound_salt = session.server_salt
    ...
    # Diagnostic: inbound salt must propagate so server.py can log it.
    assert err.inbound_salt == inbound_salt
    assert err.new_server_salt == session.server_salt


def test_decode_accepts_previous_salt_in_grace_window() -> None:
    """In-flight client messages encrypted with the previously-current salt
    must keep validating for ``SERVER_SALT_GRACE_WINDOW_SEC`` after a rotation,
    matching MTProto REQ-SALT-08 (1800 s acceptance of old salts)."""
    store = SessionStore()
    session = _make_session(store, session_id=106, new_nonce=4242, server_nonce=9090)
    msg_id = _valid_msg_id()
    client_mtproto_session = 0xCAFE_1234

    encoded = encode_encrypted_message(
        session, client_mtproto_session, msg_id, 1, b"grace--!",
    )

    # Simulate a rotation: capture the salt that the client used and force a
    # rotation that picks a brand-new salt as ``current``.
    prev_salt = session.server_salt
    now = int(time.time())
    session.salt.server_salt_valid_until = now - 1
    session.salt.accepted_future_salts[0xABCD_FACE] = (
        now - 10, now + 3600,
    )
    session.salt.rotate_if_needed(now=now)
    assert session.server_salt != prev_salt
    assert prev_salt in session.salt.previous_salts

    # The pre-rotation client envelope must still decode cleanly.
...
def test_decode_accepts_future_salt_registered_via_get_future_salts() -> None:
    """Inbound header salt may match the promoted scheduled salt window."""
    store = SessionStore()
    ...
    session.server_salt ^= 0xFFFFFFFFFFFFFFFF
    session.server_salt_valid_until = now - 1
