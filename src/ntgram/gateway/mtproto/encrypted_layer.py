...
from ntgram.gateway.mtproto.aes import aes_ige_decrypt, aes_ige_encrypt, xor_bytes
...
class EncryptedLayerError(ValueError):
    ...
    def __init__(
        ...
        *,
        ...
        self.inbound_salt = inbound_salt


@dataclass(slots=True, frozen=True)
class DecodedEnvelope:
    """Result of pure (session-independent) MTProto envelope decoding."""
...
def encode_encrypted_message(
    ...
    encrypted = aes_ige_encrypt(plaintext, aes_key, aes_iv)
...
def encode_bind_temp_auth_key_inner_message(
    ...
    *,
    ...
    """Encode auth.bindTempAuthKey encrypted_message."""
...
def decode_bind_temp_auth_key_inner_message(
    ...
    *,
    ...
    """Decode auth.bindTempAuthKey encrypted_message."""
    if len(payload) < 24:
...
def decode_encrypted_message_pure(auth_key: bytes, payload: bytes) -> DecodedEnvelope:
    """Pure cryptographic decode of an MTProto encrypted envelope."""
    if len(payload) < 24:
        raise EncryptedLayerError("encrypted payload too short")
    auth_key_id = struct.unpack("<Q", payload[:8])[0]
    msg_key = payload[8:24]
    ...
    if len(ciphertext) == 0 or len(ciphertext) % 16 != 0:
        raise EncryptedLayerError("invalid encrypted body size")
    aes_key, aes_iv = _kdf_aes_key_iv(auth_key, msg_key, x=0)
    plaintext = aes_ige_decrypt(ciphertext, aes_key, aes_iv)
    expected_msg_key = _compute_msg_key(auth_key, plaintext, x=0)
    if expected_msg_key != msg_key:
        raise EncryptedLayerError("msg_key mismatch")
    if len(plaintext) < 32:
        raise EncryptedLayerError("plaintext too short")
...
def validate_envelope_against_session(
    session: AuthSession,
    envelope: DecodedEnvelope,
) -> None:
    """Read-only session checks on an already-decoded envelope."""
    if envelope.auth_key_id != session.auth_key_id:
        raise EncryptedLayerError("auth_key_id mismatch")
    if not session.is_accepted_server_salt(envelope.server_salt):
        raise EncryptedLayerError(
            "bad_server_salt",
            error_code=48,
            bad_msg_id=envelope.msg_id,
            bad_msg_seqno=envelope.seq_no,
            new_server_salt=session.server_salt,
            context_session_id=envelope.session_id,
            inbound_salt=envelope.server_salt,
        )
    _validate_client_msg_id(envelope.msg_id, envelope.seq_no)


def commit_envelope_to_session(
    session: AuthSession,
    envelope: DecodedEnvelope,
) -> None:
    """Apply an envelope's side-effects to the session."""
    if not session.bind_mtproto_session(envelope.session_id):
        raise EncryptedLayerError(
            "session_id mismatch",
            error_code=64,
            bad_msg_id=envelope.msg_id,
    ...
    if not session.touch_msg_id(envelope.msg_id):
        raise EncryptedLayerError(
...
def decode_encrypted_message(
    ...
    """Backward-compatible orchestrator: pure decode -> validate -> commit."""
    envelope = decode_encrypted_message_pure(session.auth_key, payload)
    validate_envelope_against_session(session, envelope)
    commit_envelope_to_session(session, envelope)
    return envelope.session_id, envelope.msg_id, envelope.seq_no, envelope.message_data
